 ...
 */
contract MusicNFT is ERC721, ERC721Enumerable, Ownable, Pausable, ERC2981, Counters {
    ...
    function getTokensForSale() external view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= totalSupply; i++) {
            if (_exists[i] && musicMetadata[i].isForSale) {
        ...
        assembly {
            mstore(result, 0x20, mul(currentIndex, 0x20))
        }
    ...
    function getTokensByArtist(string memory artist) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= totalSupply; i++) {
            if (_exists[i] && keccak256(bytes(musicMetadata[i].artist)) == keccak256(bytes(artist))) {
                result[currentIndex] = i;
                currentIndex++;
            }
        }
    ...
    function getTokensByGenre(string memory genre) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= totalSupply; i++) {
            if (_exists[i] && keccak256(bytes(musicMetadata[i].genre)) == keccak256(bytes(genre))) {
                result[currentIndex] = i;
                currentIndex++;
            }
        }
    ...
    function _initializeCounter() internal initializer {
        __Counter_init(_tokenIdCounter._slotId, 1);
    }
